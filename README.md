...
```bash
pip install -e .              # python-jobspy + tenacity are pinned in pyproject.toml
...
| Stage | What Happens |
|-------|-------------|
| **1. Discover** | Scrapes JobSpy boards (Indeed, LinkedIn, Naukri, Google Jobs, etc.) + Workday employer portals + Greenhouse ATS employers + SmartRecruiters ATS employers + direct career sites. Followed by a deterministic rule gate and a cheap LLM sweep that mark out-of-scope rows before enrichment |
...
### `searches.yaml`
Job search queries, locations, JobSpy `sites` list, and per-job filtering knobs:

- `queries` — tiered search terms (tier 1/2/3)
- `locations` — list of `{location, remote}` entries
- `sites` — JobSpy backends to enable (`indeed, linkedin, naukri, google, glassdoor, zip_recruiter, bayt, bdjobs`)
- `country` / `country_indeed` — country routing for ATS APIs
- `location_accept` / `location_reject_non_remote` — substring location filters
...
### `llm.yaml`
Per-task LLM routing (different model/provider per pipeline stage). Tasks: `prefilter`, `discover`, `enrich`, `score`, `tailor`, `cover`. Each task can set `model`, `fallback`, `api_base`, `api_key_env`, plus matching `fallback_*` fields. If absent, every task uses the `default` model.

### `.env`
API keys and runtime config: `GEMINI_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `LIGHTNING_API_KEY`, `LLM_URL`, optional `LLM_MODEL`, optional `LLM_API_KEY`, and `CAPSOLVER_API_KEY`.
...
### Discover
Queries the JobSpy backends configured in `searches.yaml` (Indeed, LinkedIn, Naukri, Google Jobs, Glassdoor, ZipRecruiter, Bayt, BDJobs). Fetches from Greenhouse ATS, SmartRecruiters ATS, and Workday employer portals. Hits direct career sites with custom extractors. Deduplicates by URL.
...
For YAML-level scoping use `discover_backends` (subset of `[jobspy, workday, greenhouse, smartrecruiters, smartextract]`) and `discover_tags` (only crawl employers whose registry entry has matching tags).
